...
public class RedisPublishingMessageHandlerTests {
	...
	public void test() throws Exception {
		...
		container.addMessageListener(listener, Collections.<Topic>singletonList(new ChannelTopic(redisChannelName)));
		container.start();
		Thread.sleep(500);

		final RedisPublishingMessageHandler handler = new RedisPublishingMessageHandler(connectionFactory);
		handler.setDefaultRedisChannel(redisChannelName);

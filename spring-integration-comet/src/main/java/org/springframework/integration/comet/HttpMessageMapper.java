...
import org.springframework.integration.http.converter.*;
...
public class HttpMessageMapper {
	...
	private static final boolean jaxb2Present = ClassUtils.isPresent( "javax.xml.bind.Binder", HttpMessageMapper.class.getClassLoader());

	private static final boolean jacksonPresent =
			ClassUtils.isPresent( "org.codehaus.jackson.map.ObjectMapper", HttpMessageMapper.class.getClassLoader())
			&& ClassUtils.isPresent("org.codehaus.jackson.JsonGenerator", HttpMessageMapper.class.getClassLoader());
	...
	/**
	 * @return whether the request has a readable body (not a GET, HEAD, or OPTIONS request) and a Content-Type header.
	 */
	private boolean isReadable(ServletServerHttpRequest request) {
		...
		return !(HttpMethod.GET.equals(method) || HttpMethod.HEAD.equals(method) ||
				HttpMethod.OPTIONS.equals(method)) && request.getHeaders().getContentType() != null;
	}
